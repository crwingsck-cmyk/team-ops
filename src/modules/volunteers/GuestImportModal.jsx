...
const FIELDS = [
  ...
  { key: "area", label: "居住地區", keywords: ["地區", "地址", "Address", "Area"] },
...
export default function GuestImportModal({ onImport, onClose }) {
  ...
  useEffect(() => {
    ...
    FIELDS.forEach((f) => { initialMapping[f.key] = guessColumn(cols, f.keywords); });
    setMapping(initialMapping);
  }, [sheet, headerRow]);

  const buildGuest = (row) => {
    const data = {};
    FIELDS.forEach((f) => {
  ...
  const preview = step === "preview" ? rows.map(buildGuest) : [];
  const validPreview = preview.filter((g) => g.name);
  const skippedCount = preview.length - validPreview.length;

  const runImport = async () => {
    setStep("importing");
    const toImport = rows.map(buildGuest).filter((g) => g.name);
    ...
    <div className="space-y-4">
      ...
      {step === "preview" && (
        <div>
          ...
          <div className="max-h-[45vh] overflow-y-auto border border-slate-100 rounded-xl divide-y divide-slate-100">
            {validPreview.slice(0, 50).map((g, i) => (
              <div key={i} className="px-4 py-2 text-sm">
                <span className="font-bold text-slate-800">{g.name}</span>
                <span className="text-slate-400 ml-2">{g.phone} {g.area}</span>
              </div>
            ))}
      ...
      {step === "done" && (
        <div className="text-center py-8">
          ...
          <p className="text-slate-700 font-bold mb-4">已成功匯入 {progress.total} 筆大德資料。</p>
